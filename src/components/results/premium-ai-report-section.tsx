...
function getErrorMessage(code?: string | null) {
  switch (code) {
    ...
    case "invalid_ai_response":
      return "Der KI-Bericht konnte nicht sicher ausgewertet werden. Bitte versuche es spaeter erneut.";
    case "openrouter_key_missing":
      return "Die KI-Beratung ist gerade nicht verfuegbar. Bitte versuche es spaeter noch einmal.";
    case "missing_analysis_id":
    case "analysis_not_found":
      return "Die passende Analyse wurde nicht gefunden. Starte die Seite bitte neu oder oeffne den Ergebnislink erneut.";
    case "storage_error":
    case "save_failed":
    case "internal_error":
    ...
    <div className="mt-6 space-y-6">
      <div className="grid gap-4 lg:grid-cols-[1.05fr_0.95fr]">
        <article className="rounded-2xl border border-cyan-200 bg-white p-5 shadow-[0_20px_80px_-58px_rgba(15,23,42,0.45)]">
          ...
          <p className="mt-3 text-base leading-8 text-slate-700">{report.executiveSummary}</p>
        </article>
        ...
        <article className="rounded-2xl border border-amber-200 bg-white p-5 shadow-[0_20px_80px_-58px_rgba(15,23,42,0.45)]">
          ...
          <p className="mt-3 text-base leading-8 text-slate-700">{report.mainDiagnosis}</p>
          {source ? (
      ...
      <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_18px_70px_-55px_rgba(15,23,42,0.45)]">
        <h3 className="text-xl font-bold text-slate-950">Score-Erklaerung</h3>
        <p className="mt-3 text-sm leading-7 text-slate-700">{report.scoreExplanation}</p>
      </article>

      <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_18px_70px_-55px_rgba(15,23,42,0.45)]">
        <h3 className="text-xl font-bold text-slate-950">Top Issues</h3>
            ...
            <div key={`${issue.title}-${index}`} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  <h4 className="mt-2 text-lg font-bold text-slate-950">{issue.title}</h4>
                </div>
              ...
              <p className="mt-3 text-sm leading-7 text-slate-700">{issue.whyItMatters}</p>
              ...
              <p className="mt-3 text-sm font-bold leading-7 text-slate-950">
                Empfehlung: {issue.recommendedAction}
              </p>
            </div>
          ))}
        </div>
      ...
      <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_18px_70px_-55px_rgba(15,23,42,0.45)]">
        <h3 className="text-xl font-bold text-slate-950">Massnahmenplan</h3>
    ...
    return (
      <section className="rounded-[2rem] border border-white/10 bg-slate-900 p-6 text-white shadow-[0_30px_90px_-60px_rgba(15,23,42,0.9)] sm:p-8">
        ...
        <p className="mt-3 max-w-2xl text-sm leading-7 text-slate-300">
          Nach dem Premium-Upgrade kannst du den gespeicherten KI-Bericht fuer diese Analyse erzeugen und erneut lesen.
        </p>
      </section>
    );
  }
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div className="max-w-3xl">
            ...
            <p className="mt-4 max-w-2xl text-base leading-8 text-slate-200">
              Eine zusaetzliche Beratungs-Ebene mit Diagnose, Prioritaeten und konkreten Textideen.
            </p>
          </div>
        ...
        {state === "idle" && !report ? (
          <div className="rounded-2xl border border-cyan-200 bg-white p-5 shadow-[0_18px_70px_-55px_rgba(15,23,42,0.45)]">
            ...
            <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-700">
              Der Bericht wird erst erzeugt, wenn du den Button klickst. Falls fuer diese Analyse bereits ein KI-Bericht gespeichert ist, wird er direkt wiederverwendet.
            </p>
          </div>
        ) : null}
        ...
        {state === "loading" ? (
          <div className="rounded-2xl border border-cyan-200 bg-white p-5 shadow-[0_18px_70px_-55px_rgba(15,23,42,0.45)]">
            ...
            <p className="mt-3 text-sm leading-7 text-slate-700">
              Das kann einen Moment dauern. Bitte lasse diese Seite geoeffnet; ein zweiter Auftrag wird nicht parallel gestartet.
            </p>
          </div>
        ) : null}
        ...
        {state === "error" ? (
          <div className="rounded-2xl border border-rose-200 bg-rose-50 p-5 text-rose-950 shadow-[0_18px_70px_-55px_rgba(127,29,29,0.35)]">
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-rose-700">
              KI-Bericht nicht verfuegbar
            </p>
            <h3 className="mt-3 text-xl font-bold">Bitte spaeter erneut versuchen</h3>
